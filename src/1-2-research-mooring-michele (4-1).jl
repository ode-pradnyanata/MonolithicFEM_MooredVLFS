...
function run_Mooring(case::Mooring_case_params)
  ...
  x, η, ω, vₚ, Cw = run_mooring_case_freq_domain(case)
  ...
  push!(data,"x"=>x, "η"=>η, "ω"=>ω, "vₚ"=>vₚ, "Cw"=>Cw)  
  save(file,data)
  return data
end
...
function run_mooring_case_freq_domain(params::Mooring_case_params) 
  ...
  function is_beam4(xs) # Check if an element is inside the beam4
    n = length(xs)
    x = (1/n)*sum(xs)
    (xbⱼ₃ <= x[1] <= xbⱼ₄ ) * ( x[2] ≈ H)
  end
  function is_beam5(xs) # Check if an element is inside the beam5
    n = length(xs)
    x = (1/n)*sum(xs)
    (xbⱼ₄ <= x[1] <= xbⱼ₅ ) * ( x[2] ≈ H)
  end
  function is_beam6(xs) # Check if an element is inside the beam6
    n = length(xs)
    x = (1/n)*sum(xs)
    (xbⱼ₅ <= x[1] <= xb₁ ) * ( x[2] ≈ H)
  end      
  ...
  function is_joint2(xs) # Check if an element is a joint
    ...
    function is_joint3(xs) # Check if an element is a joint
    is_on_xbⱼ₃ = [x[1]≈xbⱼ₃ && x[2]≈H for x in xs] 
    element_on_xbⱼ₃ = minimum(is_on_xbⱼ₃) 
    element_on_xbⱼ₃
  end
    function is_joint4(xs) # Check if an element is a joint
    is_on_xbⱼ₄ = [x[1]≈xbⱼ₄ && x[2]≈H for x in xs] 
    element_on_xbⱼ₄ = minimum(is_on_xbⱼ₄) 
    element_on_xbⱼ₄
  end
    function is_joint5(xs) # Check if an element is a joint
    is_on_xbⱼ₅ = [x[1]≈xbⱼ₅ && x[2]≈H for x in xs] 
    element_on_xbⱼ₅ = minimum(is_on_xbⱼ₅) 
    element_on_xbⱼ₅
  end
  ...
  Γb4_to_Γ_mask = lazy_map(is_beam4,xΓ) 
  Γb5_to_Γ_mask = lazy_map(is_beam5,xΓ) 
  Γb6_to_Γ_mask = lazy_map(is_beam6,xΓ) 
  ...
  Γb4_to_Γ = findall(Γb4_to_Γ_mask)
  Γb5_to_Γ = findall(Γb5_to_Γ_mask)
  Γb6_to_Γ = findall(Γb6_to_Γ_mask)
  Γd1_to_Γ = findall(Γd1_to_Γ_mask)
  Γd2_to_Γ = findall(Γd2_to_Γ_mask)
  Γf_to_Γ = findall(!,Γb1_to_Γ_mask .| Γb2_to_Γ_mask .| Γb3_to_Γ_mask .| Γb4_to_Γ_mask .| Γb5_to_Γ_mask .| Γb6_to_Γ_mask .| Γd1_to_Γ_mask .| Γd2_to_Γ_mask)
  Γη_to_Γ = findall(Γb1_to_Γ_mask .| Γb2_to_Γ_mask .| Γb3_to_Γ_mask .| Γb4_to_Γ_mask .| Γb5_to_Γ_mask .| Γb6_to_Γ_mask)
  Γκ_to_Γ = findall(!,Γb1_to_Γ_mask .| Γb2_to_Γ_mask .| Γb3_to_Γ_mask .| Γb4_to_Γ_mask .| Γb5_to_Γ_mask .| Γb6_to_Γ_mask)
  Γb1 = Triangulation(Γ,Γb1_to_Γ)
  Γb2 = Triangulation(Γ,Γb2_to_Γ)
  Γb3 = Triangulation(Γ,Γb3_to_Γ)
  Γb4 = Triangulation(Γ,Γb4_to_Γ)
  Γb5 = Triangulation(Γ,Γb5_to_Γ)
  Γb6 = Triangulation(Γ,Γb6_to_Γ)
...
# 1 Beam joints
  ...
  Λj3_to_Γ_mask = lazy_map(is_joint3,xΓ_dim_0)
  Λj4_to_Γ_mask = lazy_map(is_joint4,xΓ_dim_0)
  Λj5_to_Γ_mask = lazy_map(is_joint5,xΓ_dim_0)

  Λj1_to_Γ = findall(Λj1_to_Γ_mask)
  Λj2_to_Γ = findall(Λj2_to_Γ_mask)
  Λj3_to_Γ = findall(Λj3_to_Γ_mask)
  Λj4_to_Γ = findall(Λj4_to_Γ_mask)
  Λj5_to_Γ = findall(Λj5_to_Γ_mask)
  Λj1 = Skeleton(Γ,Λj1_to_Γ_mask)
  Λj2 = Skeleton(Γ,Λj2_to_Γ_mask)
  Λj3 = Skeleton(Γ,Λj3_to_Γ_mask)
  Λj4 = Skeleton(Γ,Λj4_to_Γ_mask)
  Λj5 = Skeleton(Γ,Λj5_to_Γ_mask)
  ...
  @show num_dims(Λj3)
  @show num_cells(Λj3)
  println("Node Λj3 = ",Λj3_to_Γ)
  println("located at ", get_cell_coordinates(Λj3))
  @show num_dims(Λj4)
  @show num_cells(Λj4)
  println("Node Λj4 = ",Λj4_to_Γ)
  println("located at ", get_cell_coordinates(Λj4))
  @show num_dims(Λj5)
  @show num_cells(Λj5)
  println("Node Λj5 = ",Λj5_to_Γ)
  println("located at ", get_cell_coordinates(Λj5))
...
# 2 Beam boundary conditions, left-right ends
  ...
  if vtk_output == true
    ...
    writevtk(Γb4,filename*"_Γb4")
    writevtk(Γb5,filename*"_Γb5")
    writevtk(Γb6,filename*"_Γb6")
    ...
    writevtk(Λb4,filename*"_Λb4")        
    writevtk(Λb5,filename*"_Λb5")        
    writevtk(Λb6,filename*"_Λb6")        
    writevtk(Λj1,filename*"_Λj1")
    writevtk(Λj2,filename*"_Λj2")        
    writevtk(Λj3,filename*"_Λj3")
    writevtk(Λj4,filename*"_Λj4")
    writevtk(Λj5,filename*"_Λj5")
  ...
  dΓb4 = Measure(Γb4,degree)
  dΓb5 = Measure(Γb5,degree)
  dΓb6 = Measure(Γb6,degree)
  ...
  dΛb4 = Measure(Λb4,degree)
  dΛb5 = Measure(Λb5,degree)
  dΛb6 = Measure(Λb6,degree)
  dΛj1 = Measure(Λj1,degree)
  dΛj2 = Measure(Λj2,degree)
  dΛj3 = Measure(Λj3,degree)
  dΛj4 = Measure(Λj4,degree)
  dΛj5 = Measure(Λj5,degree)
  ...
  nΛb4 = get_normal_vector(Λb4)
  nΛb5 = get_normal_vector(Λb5)
  nΛb6 = get_normal_vector(Λb6)      
  nΛj1 = get_normal_vector(Λj1)
  nΛj2 = get_normal_vector(Λj2)
  nΛj3 = get_normal_vector(Λj3)
  nΛj4 = get_normal_vector(Λj4)
  nΛj5 = get_normal_vector(Λj5)
  ...
  a((ϕ,κ,η),(w,u,v)) = (∫(  ∇(w)⋅∇(ϕ) )dΩ   +
    ...
    ∫(  ( Dᵨ*Δ(v)*Δ(η) - A2*(∇(v)⋅∇(η)) + A3*v*η - A4*(∇(v)⋅∇(ϕ)) + A5*v*ϕ  + Tᵨ*(∇(v)⋅∇(η))) +  im*ω*w*η )dΓb4 +
    ∫(  ( Dᵨ*Δ(v)*Δ(η) - A2*(∇(v)⋅∇(η)) + A3*v*η - A4*(∇(v)⋅∇(ϕ)) + A5*v*ϕ  + Tᵨ*(∇(v)⋅∇(η))) +  im*ω*w*η )dΓb5 +
    ∫(  ( Dᵨ*Δ(v)*Δ(η) - A2*(∇(v)⋅∇(η)) + A3*v*η - A4*(∇(v)⋅∇(ϕ)) + A5*v*ϕ  + Tᵨ*(∇(v)⋅∇(η))) +  im*ω*w*η )dΓb6 +
    ...
    ∫(  (Dᵨ) * ( - jump(∇(v)⋅nΛb4) * mean(Δ(η)) - mean(Δ(v)) * jump(∇(η)⋅nΛb4) + γ*( jump(∇(v)⋅nΛb4) * jump(∇(η)⋅nΛb4) ) ) )dΛb4 +
    ∫(  (Dᵨ) * ( - jump(∇(v)⋅nΛb5) * mean(Δ(η)) - mean(Δ(v)) * jump(∇(η)⋅nΛb5) + γ*( jump(∇(v)⋅nΛb5) * jump(∇(η)⋅nΛb5) ) ) )dΛb5 +    
    ∫(  (Dᵨ) * ( - jump(∇(v)⋅nΛb6) * mean(Δ(η)) - mean(Δ(v)) * jump(∇(η)⋅nΛb6) + γ*( jump(∇(v)⋅nΛb6) * jump(∇(η)⋅nΛb6) ) ) )dΛb6 +
    ...
    ∫(  (mean(v) * ks2ᵨ * mean(η)) + (mean(v) * (-im*ω*vᵨ) * mean(η)) + (-Dᵨ*jump(∇(v)⋅nΛj3)*mean(Δ(η))) +
          (-Dᵨ*jump(∇(η)⋅nΛj3)*mean(Δ(v)))  - (-Dᵨ*γ*(jump(∇(v)⋅nΛj3) * jump(∇(η)⋅nΛj3)))  )dΛj3  +
    ∫(  (mean(v) * ks2ᵨ * mean(η)) + (mean(v) * (-im*ω*vᵨ) * mean(η)) + (-Dᵨ*jump(∇(v)⋅nΛj4)*mean(Δ(η))) +
          (-Dᵨ*jump(∇(η)⋅nΛj4)*mean(Δ(v)))  - (-Dᵨ*γ*(jump(∇(v)⋅nΛj4) * jump(∇(η)⋅nΛj4)))  )dΛj4  +
    ∫(  (mean(v) * ks2ᵨ * mean(η)) + (mean(v) * (-im*ω*vᵨ) * mean(η)) + (-Dᵨ*jump(∇(v)⋅nΛj5)*mean(Δ(η))) +
          (-Dᵨ*jump(∇(η)⋅nΛj5)*mean(Δ(v)))  - (-Dᵨ*γ*(jump(∇(v)⋅nΛj5) * jump(∇(η)⋅nΛj5)))  )dΛj5  +
...
# -------------------------------------------------------
  ...
  # [2] Extract the solution at PTO
  # the pto location is relative to - xb₀
  idx_pto = [argmin(abs.(xs .- (xb₀-xb₀))), argmin(abs.(xs .- (xbⱼ₁-xb₀))), argmin(abs.(xs .- (xbⱼ₂-xb₀))),
           argmin(abs.(xs .- (xbⱼ₃-xb₀))),argmin(abs.(xs .- (xbⱼ₄-xb₀))),argmin(abs.(xs .- (xbⱼ₅-xb₀))),
           argmin(abs.(xs .- (xb₁-xb₀)))]
  η_pto = η_rel_xs[idx_pto] .* η₀

  Cg = (ω_opt/(2*k_opt)) * (1+ (2*k_opt*H)/(sinh(2*k_opt*H)))
  
  @show idx_pto
  @show η_pto

  # [3] Power calculations
  P_pto = 0.5 .* v_pto .* ω_opt^2 .* η_pto.^2
  Pw = 0.5 * ρ *g * η₀^2 * Cg
  Cw = sum(P_pto)/Pw
  ...
  return (xs, η_rel_xs, ω_opt, v_pto, Cw)
  # return (xs, η_rel_xs)
end
end
